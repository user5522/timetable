...
import 'package:timetable/features/settings/screens/timetable_management.dart';
import 'package:timetable/shared/widgets/bottom_sheets/subject_duration.dart';
import 'package:timetable/features/settings/providers/settings.dart';
import 'package:timetable/features/settings/screens/timetable_period.dart';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      children: [
          ...
          onTap: () {
            showModalBottomSheet<Duration>(
              ...
              builder: (context) {
                return Wrap(
                  children: [
                    SubjectDurationModalBottomSheet(duration: duration),
                  ],
                );
              },
            ).then(
        ...
        SwitchListTile(
          secondary: const Icon(Icons.rotate_90_degrees_ccw_outlined, size: 20),
          visualDensity: const VisualDensity(horizontal: -4),
          title: const Text("rotation_week").plural(2),
          value: rotationWeeks,
          onChanged: (bool value) => settings.updateRotationWeeks(value),
        ),
        SwitchListTile(
          secondary: const Icon(Icons.format_color_fill_outlined, size: 20),
          visualDensity: const VisualDensity(horizontal: -4),
          title: const Text("auto_complete_colors").tr(),
          subtitle: const Text("auto_complete_colors_description").tr(),
          value: autoCompleteColor,
          onChanged: (bool value) => settings.updateAutoCompleteColor(value),
        ),
      ],
    );
  }
